<script>
  ...
  export async function set(data) {
    ...
    await $module_editor?.set({
      code: data.files[0].source,
      lang: data.files[0].type,
    });
  ...
  export function update(data) {
    ...
    const matched_file = data.files.find(
      (file) => get_full_filename(file) === $selected_name
    );

    console.log(match);

    $selected_name = matched_file
      ? get_full_filename(matched_file)
      : "App.svelte";
  ...
  async function handle_change(event) {
    ...
    files.update(($files) => {
      ...
      const idx = $files.findIndex(
        (val) => get_full_filename(val) === $selected_name
      );
